...
async function getArea() {
    const res = await (await fetch('https://www.themealdb.com/api/json/v1/1/list.php?a=list')).json();
    console.log(res)
    return res;
}
display()
...
function displayArea(areaData) {
    ...
    areaData.meals.forEach(meal => {
        ...
        <div class="col-md-3 border-0">
            ...
            <div class='card-area bg-dark' style=" cursor: pointer;" id="${meal.strArea}">
            <i class="fa-solid fa-house-laptop w-100 fa-5x h-100 text-light  "></i>
                
             <h3 class=" text-light">${meal.strArea}</h3>
             </div>      
    ...
    async function getFilterArea(filter) {
        const resp = await (await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${filter}`)).json();
        console.log(resp);
        return resp;
    }
